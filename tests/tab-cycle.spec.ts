...
const expectTabCycleNonsingular = async ({ page }) => {
  await page.keyboard.press("Tab");
  // the first element we tab to will be used as point of comparison
  const referenceElementHandle = await page.evaluateHandle(
    "document.activeElement"
  );
  let encounteredFirstElementAgain = false;
  let encounteredOtherElement = false;
  // i < 100 is arbitrary, but this test should certainly fail if the user has
  // to tab more 100 times to return to the top
  for (let i = 0; i < 100; i++) {
    await page.keyboard.press("Tab");
    const sameActiveElement = (el) => el === document.activeElement;
    // is the current element that we have tabbed to the same as the first?
    if (await page.evaluate(sameActiveElement, referenceElementHandle)) {
      encounteredFirstElementAgain = true;
    }
    // is the current element that we have tabbed to the different from the first?
    else {
      encounteredOtherElement = true;
    }
    if (encounteredFirstElementAgain) {
      referenceElementHandle.dispose();
      break;
    }
  }
  expect(
...
test.describe("when pressing tab key repeatedly", () => {
  test.describe("should cycle through elements back to the start", () => {});

  test("should succeed on a simple page with no tab traps", async ({
    page,
  }) => {
    await page.goto("http://127.0.0.1:8080/two-tabbable.pass.html");
    await expectTabCycleNonsingular({ page });
  });

  test("should pass on page with a single tabbable element", async ({
    page,
  }) => {
    await page.goto("http://127.0.0.1:8080/one-tabbable.pass.html");
    await expectTabCycleNonsingular({ page });
  });

  test("should fail on page with simple tab trap", async ({ page }) => {
    test.fail();
    await page.goto("http://127.0.0.1:8080/simple-tab-trap.fail.html");
    await expectTabCycleNonsingular({ page });
  });
